...
[`SyncPart`](/api/#pyqwest.SyncPart) to set a filename or part headers such as a
...
raised, because the rest of the content is lost. When a failure or response arrives while
an attempt is waiting on the content for its next chunk, a retry replays what was already
read and then waits for that chunk. `RetryMode.UNBUFFERED` retains no streamed content and
makes streamed requests only retry connection errors. Content provided as `bytes` is
